...
def get_border_irregularity(mask, img=None):
    """
    Compares the lesion's perimeter to the perimeter a perfect circle
    with the same area would have. A smooth round lesion ≈ 1. Jagged
    irregular border > 1.
    """
    ...
    # perimeter via erosion (same as feature_shape, maybe refactor later)
...
# border gradient — we added this part
def get_border_gradient(mask, img):
    ...
    # get the border pixels (same erosion trick as before)
    eroded = morphology.erosion(binary_mask)
