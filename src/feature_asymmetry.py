...
def get_asymmetry(mask, img=None):
    """
    Asymmetry score: flip the mask horizontally and vertically, see how
    much overlap there is.

    0 = symmetric, higher = more asymmetric (worse, generally).

    First version of this just flipped the raw mask which gave nonsense
    for off-center lesions (because flipping happened across the image
    edge, not the lesion center). Now we crop to the lesion bbox first.
    """
